...
public:
	...
	bool put(Slice &key, Slice &value, int, int, int, int); //returns true if value overwritten
	bool del(Slice &key, int, int, int, int);
...
kvStore::kvStore(uint64_t max_entries)
{
	...
	nodes[0].nxt = -1;
	for (int i = 1; i < 312 * max_entries - 1; i++)
		nodes[i].nxt = i + 1;
	free_head = 1;
...
bool kvStore::put(Slice &key, Slice &value, int i = 0, int cur = 0, int l = 0, int x = 0)
{
	...
	nodes[cur].nxt = -1;
	nodes[cur].ends++;
	return put(key, value, i, cur, l + 1, x >> 1);
}
...
bool kvStore::del(Slice &key, int i = 0, int cur = 0, int l = 0, int x = 0)
{
	...
	if (nodes[cur].arr[x & 1])
		...
		return false;
	bool ret = del(key, i, cur, l + 1, x >> 1);
	nodes[cur].ends -= ret;
